...
//import intel_pcie_axi_mcdma_pkg::*;

module intel_pcie_axi_mcdma_top
  #(
  ...
  localparam ENABLE_RESET_SYNC = 0;

  logic  axi_st_areset_n_wire;
  logic  axi_lite_areset_n_wire;
  logic  axi_mm_areset_n_wire;

  generate if (ENABLE_RESET_SYNC) begin : gen_enable_reset_sync

     // AXI-st clock domain
     intel_pcie_axi_mcdma_reset_sync #(
        .SYNC_W         (4)
     ) axi_st_areset_n_sync_i (
        .aclk           (axi_st_clk),
        .async_areset_n (axi_st_areset_n),
        .sync_areset_n  (axi_st_areset_n_wire)
     );

     // AXI-lite clock domain -- lite clock is 1/4th of st clock
     intel_pcie_axi_mcdma_reset_sync #(
        .SYNC_W         (4)
     ) axi_lite_areset_n_sync_i (
        .aclk           (axi_lite_clk),
        .async_areset_n (axi_lite_areset_n),
        .sync_areset_n  (axi_lite_areset_n_wire)
     );

     // AXI-mm clock domain
     intel_pcie_axi_mcdma_reset_sync #(
        .SYNC_W         (4)
     ) axi_mm_areset_n_sync_i (
        .aclk           (axi_mm_clk),
        .async_areset_n (axi_mm_areset_n),
        .sync_areset_n  (axi_mm_areset_n_wire)
     );

  end else begin : gen_disable_reset_sync

     assign axi_st_areset_n_wire    = axi_st_areset_n;
     assign axi_lite_areset_n_wire  = axi_lite_areset_n;
     assign axi_mm_areset_n_wire    = axi_mm_areset_n;

  end endgenerate

  intel_pcie_axi_mcdma_wrapper #(
     ...
     // Control and Status Register Responder Interface (axi_lite_clk)
     .user_dma_lite_csr_awvalid     ( usr_hip_reconfig_awvalid ),
     .dma_user_lite_csr_awready     ( usr_hip_reconfig_awready ),
     .user_dma_lite_csr_awaddr      ( usr_hip_reconfig_awaddr  ),
     .user_dma_lite_csr_wvalid      ( usr_hip_reconfig_wvalid  ),
     .dma_user_lite_csr_wready      ( usr_hip_reconfig_wready  ),
     .user_dma_lite_csr_wdata       ( usr_hip_reconfig_wdata   ),
     .user_dma_lite_csr_wstrb       ( usr_hip_reconfig_wstrb   ),
     .dma_user_lite_csr_bvalid      ( usr_hip_reconfig_bvalid  ),
     .user_dma_lite_csr_bready      ( usr_hip_reconfig_bready  ),
     .dma_user_lite_csr_bresp       ( usr_hip_reconfig_bresp   ),
     .user_dma_lite_csr_arvalid     ( usr_hip_reconfig_arvalid ),
     .dma_user_lite_csr_arready     ( usr_hip_reconfig_arready ),
     .user_dma_lite_csr_araddr      ( usr_hip_reconfig_araddr  ),
     .dma_user_lite_csr_rvalid      ( usr_hip_reconfig_rvalid  ),
     .user_dma_lite_csr_rready      ( usr_hip_reconfig_rready  ),
     .dma_user_lite_csr_rdata       ( usr_hip_reconfig_rdata   ),
     .dma_user_lite_csr_rresp       ( usr_hip_reconfig_rresp   ),

     // User Side AXI-MM DMA Interface
     .dma_user_mm_initatr_awvalid   ( dma_axi_mm_awvalid ),
     .user_dma_mm_initatr_awready   ( dma_axi_mm_awready ),
     .dma_user_mm_initatr_awid      ( dma_axi_mm_awid    ),
     .dma_user_mm_initatr_awaddr    ( dma_axi_mm_awaddr  ),
     .dma_user_mm_initatr_awlen     ( dma_axi_mm_awlen   ),
     .dma_user_mm_initatr_awsize    ( dma_axi_mm_awsize  ),
     .dma_user_mm_initatr_awburst   ( dma_axi_mm_awburst ),
     .dma_user_mm_initatr_awprot    ( dma_axi_mm_awprot  ),
     .dma_user_mm_initatr_awlock    ( dma_axi_mm_awlock  ),
     .dma_user_mm_initatr_wvalid    ( dma_axi_mm_wvalid  ),
     .dma_user_mm_initatr_wlast     ( dma_axi_mm_wlast   ),
     .user_dma_mm_initatr_wready    ( dma_axi_mm_wready  ),
     .dma_user_mm_initatr_wdata     ( dma_axi_mm_wdata   ),
     .dma_user_mm_initatr_wstrb     ( dma_axi_mm_wstrb   ),
     .user_dma_mm_initatr_bvalid    ( dma_axi_mm_bvalid  ),
     .dma_user_mm_initatr_bready    ( dma_axi_mm_bready  ),
     .user_dma_mm_initatr_bid       ( dma_axi_mm_bid     ),
     .user_dma_mm_initatr_bresp     ( dma_axi_mm_bresp   ),
     .dma_user_mm_initatr_arvalid   ( dma_axi_mm_arvalid ),
     .user_dma_mm_initatr_arready   ( dma_axi_mm_arready ),
     .dma_user_mm_initatr_arid      ( dma_axi_mm_arid    ),
     .dma_user_mm_initatr_araddr    ( dma_axi_mm_araddr  ),
     .dma_user_mm_initatr_arlen     ( dma_axi_mm_arlen   ),
     .dma_user_mm_initatr_arsize    ( dma_axi_mm_arsize  ),
     .dma_user_mm_initatr_arburst   ( dma_axi_mm_arburst ),
     .dma_user_mm_initatr_arprot    ( dma_axi_mm_arprot  ),
     .dma_user_mm_initatr_arlock    ( dma_axi_mm_arlock  ),
     .user_dma_mm_initatr_rvalid    ( dma_axi_mm_rvalid  ),
     .user_dma_mm_initatr_rlast     ( dma_axi_mm_rlast   ),
     .dma_user_mm_initatr_rready    ( dma_axi_mm_rready  ),
     .user_dma_mm_initatr_rid       ( dma_axi_mm_rid     ),
     .user_dma_mm_initatr_rdata     ( dma_axi_mm_rdata   ),
     .user_dma_mm_initatr_rresp     ( dma_axi_mm_rresp   ),

     // User Side AXI-stream Master (H2D) Interface
     .h2d_user_st_initatr_tvalid          ( h2d_axi_st_tvalid ),
     .user_h2d_st_initatr_tready          ( h2d_axi_st_tready ),
     .h2d_user_st_initatr_tdata           ( h2d_axi_st_tdata  ),
     .h2d_user_st_initatr_tkeep           ( h2d_axi_st_tkeep  ),
     .h2d_user_st_initatr_tlast           ( h2d_axi_st_tlast  ),
     .h2d_user_st_initatr_tuser_error     ( h2d_axi_st_tuser_error      ),
     .h2d_user_st_initatr_tuser_metadata  ( h2d_axi_st_tuser_metadata   ),
     .h2d_user_st_initatr_tid             ( h2d_axi_st_tid    ),

     // User Side AXI-stream Slave (D2H) Interface
     .user_d2h_st_respndr_tvalid          ( d2h_axi_st_tvalid ),
     .d2h_user_st_respndr_tready          ( d2h_axi_st_tready ),
     .user_d2h_st_respndr_tdata           ( d2h_axi_st_tdata  ),
     .user_d2h_st_respndr_tkeep           ( d2h_axi_st_tkeep  ),
     .user_d2h_st_respndr_tlast           ( d2h_axi_st_tlast  ),
     .user_d2h_st_respndr_tuser_error     ( d2h_axi_st_tuser_error      ),
     .user_d2h_st_respndr_tuser_metadata  ( d2h_axi_st_tuser_metadata   ),
     .user_d2h_st_respndr_tid             ( d2h_axi_st_tid    ),

     // FLR interface to application/user side
     .dma_user_flr_rcvd_val         ( user_flr_rcvd_val      ),
     .dma_user_flr_rcvd_chan_num    ( user_flr_rcvd_chan_num ),
     .user_dma_flr_completed        ( user_flr_completed     ),

     // MSI-X Request interface from application/user side
     .dma_user_event_msix_tready    ( user_event_msix_tready ),
     .user_dma_event_msix_tvalid    ( user_event_msix_tvalid ),
     .user_dma_event_msix_tdata     ( user_event_msix_tdata  ),

     // User Side PIO AXI Lite Initiator Interface (axi_lite_clk)
     .pio_user_lite_initatr_awvalid ( rx_pio_axi_lite_awvalid ),
     .user_pio_lite_initatr_awready ( rx_pio_axi_lite_awready ),
     .pio_user_lite_initatr_awaddr  ( rx_pio_axi_lite_awaddr  ),
     .pio_user_lite_initatr_wvalid  ( rx_pio_axi_lite_wvalid  ),
     .user_pio_lite_initatr_wready  ( rx_pio_axi_lite_wready  ),
     .pio_user_lite_initatr_wdata   ( rx_pio_axi_lite_wdata   ),
     .pio_user_lite_initatr_wstrb   ( rx_pio_axi_lite_wstrb   ),
     .user_pio_lite_initatr_bvalid  ( rx_pio_axi_lite_bvalid  ),
     .pio_user_lite_initatr_bready  ( rx_pio_axi_lite_bready  ),
     .user_pio_lite_initatr_bresp   ( rx_pio_axi_lite_bresp   ),
     .pio_user_lite_initatr_arvalid ( rx_pio_axi_lite_arvalid ),
     .user_pio_lite_initatr_arready ( rx_pio_axi_lite_arready ),
     .pio_user_lite_initatr_araddr  ( rx_pio_axi_lite_araddr  ),
     .user_pio_lite_initatr_rvalid  ( rx_pio_axi_lite_rvalid  ),
     .pio_user_lite_initatr_rready  ( rx_pio_axi_lite_rready  ),
     .user_pio_lite_initatr_rdata   ( rx_pio_axi_lite_rdata   ),
     .user_pio_lite_initatr_rresp   ( rx_pio_axi_lite_rresp   ),

     // User Side CS AXI Lite Responder Interface (axi_lite_clk)
     .user_cs_lite_respndr_awvalid  ( cs_axi_lite_awvalid ),
     .cs_user_lite_respndr_awready  ( cs_axi_lite_awready ),
     .user_cs_lite_respndr_awaddr   ( cs_axi_lite_awaddr  ),
     .user_cs_lite_respndr_wvalid   ( cs_axi_lite_wvalid  ),
     .cs_user_lite_respndr_wready   ( cs_axi_lite_wready  ),
     .user_cs_lite_respndr_wdata    ( cs_axi_lite_wdata   ),
     .user_cs_lite_respndr_wstrb    ( cs_axi_lite_wstrb   ),
     .cs_user_lite_respndr_bvalid   ( cs_axi_lite_bvalid  ),
     .user_cs_lite_respndr_bready   ( cs_axi_lite_bready  ),
     .cs_user_lite_respndr_bresp    ( cs_axi_lite_bresp   ),
     .user_cs_lite_respndr_arvalid  ( cs_axi_lite_arvalid ),
     .cs_user_lite_respndr_arready  ( cs_axi_lite_arready ),
     .user_cs_lite_respndr_araddr   ( cs_axi_lite_araddr  ),
     .cs_user_lite_respndr_rvalid   ( cs_axi_lite_rvalid  ),
     .user_cs_lite_respndr_rready   ( cs_axi_lite_rready  ),
     .cs_user_lite_respndr_rdata    ( cs_axi_lite_rdata   ),
     .cs_user_lite_respndr_rresp    ( cs_axi_lite_rresp   ),

     // User Side AXI-MM BAM Interface (axi_mm_clk)
     .bam_user_mm_initatr_awvalid   ( bam_axi_mm_awvalid ),
     .user_bam_mm_initatr_awready   ( bam_axi_mm_awready ),
     .bam_user_mm_initatr_awid      ( bam_axi_mm_awid    ),
     .bam_user_mm_initatr_awaddr    ( bam_axi_mm_awaddr  ),
     .bam_user_mm_initatr_awlen     ( bam_axi_mm_awlen   ),
     .bam_user_mm_initatr_awsize    ( bam_axi_mm_awsize  ),
     .bam_user_mm_initatr_awburst   ( bam_axi_mm_awburst ),
     .bam_user_mm_initatr_awprot    ( bam_axi_mm_awprot  ),
     .bam_user_mm_initatr_awlock    ( bam_axi_mm_awlock  ),
     .bam_user_mm_initatr_wvalid    ( bam_axi_mm_wvalid  ),
     .bam_user_mm_initatr_wlast     ( bam_axi_mm_wlast   ),
     .user_bam_mm_initatr_wready    ( bam_axi_mm_wready  ),
     .bam_user_mm_initatr_wdata     ( bam_axi_mm_wdata   ),
     .bam_user_mm_initatr_wstrb     ( bam_axi_mm_wstrb   ),
     .user_bam_mm_initatr_bvalid    ( bam_axi_mm_bvalid  ),
     .bam_user_mm_initatr_bready    ( bam_axi_mm_bready  ),
     .user_bam_mm_initatr_bid       ( bam_axi_mm_bid     ),
     .user_bam_mm_initatr_bresp     ( bam_axi_mm_bresp   ),
     .bam_user_mm_initatr_arvalid   ( bam_axi_mm_arvalid ),
     .user_bam_mm_initatr_arready   ( bam_axi_mm_arready ),
     .bam_user_mm_initatr_arid      ( bam_axi_mm_arid    ),
     .bam_user_mm_initatr_araddr    ( bam_axi_mm_araddr  ),
     .bam_user_mm_initatr_arlen     ( bam_axi_mm_arlen   ),
     .bam_user_mm_initatr_arsize    ( bam_axi_mm_arsize  ),
     .bam_user_mm_initatr_arburst   ( bam_axi_mm_arburst ),
     .bam_user_mm_initatr_arprot    ( bam_axi_mm_arprot  ),
     .bam_user_mm_initatr_arlock    ( bam_axi_mm_arlock  ),
     .user_bam_mm_initatr_rvalid    ( bam_axi_mm_rvalid  ),
     .user_bam_mm_initatr_rlast     ( bam_axi_mm_rlast   ),
     .bam_user_mm_initatr_rready    ( bam_axi_mm_rready  ),
     .user_bam_mm_initatr_rid       ( bam_axi_mm_rid     ),
     .user_bam_mm_initatr_rdata     ( bam_axi_mm_rdata   ),
     .user_bam_mm_initatr_rresp     ( bam_axi_mm_rresp   ),

     // User Side AXI-MM BAS Interface (axi_mm_clk)
     .user_bas_mm_respndr_awvalid   ( bas_axi_mm_awvalid ),
     .bas_user_mm_respndr_awready   ( bas_axi_mm_awready ),
     .user_bas_mm_respndr_awid      ( bas_axi_mm_awid    ),
     .user_bas_mm_respndr_awaddr    ( bas_axi_mm_awaddr  ),
     .user_bas_mm_respndr_awuser    ( bas_axi_mm_awuser  ),
     .user_bas_mm_respndr_awlen     ( bas_axi_mm_awlen   ),
     .user_bas_mm_respndr_awsize    ( bas_axi_mm_awsize  ),
     .user_bas_mm_respndr_awburst   ( bas_axi_mm_awburst ),
     .user_bas_mm_respndr_awprot    ( bas_axi_mm_awprot  ),
     .user_bas_mm_respndr_awlock    ( bas_axi_mm_awlock  ),
     .user_bas_mm_respndr_wvalid    ( bas_axi_mm_wvalid  ),
     .user_bas_mm_respndr_wlast     ( bas_axi_mm_wlast   ),
     .bas_user_mm_respndr_wready    ( bas_axi_mm_wready  ),
     .user_bas_mm_respndr_wdata     ( bas_axi_mm_wdata   ),
     .user_bas_mm_respndr_wstrb     ( bas_axi_mm_wstrb   ),
     .bas_user_mm_respndr_bvalid    ( bas_axi_mm_bvalid  ),
     .user_bas_mm_respndr_bready    ( bas_axi_mm_bready  ),
     .bas_user_mm_respndr_bid       ( bas_axi_mm_bid     ),
     .bas_user_mm_respndr_bresp     ( bas_axi_mm_bresp   ),
     .user_bas_mm_respndr_arvalid   ( bas_axi_mm_arvalid ),
     .bas_user_mm_respndr_arready   ( bas_axi_mm_arready ),
     .user_bas_mm_respndr_arid      ( bas_axi_mm_arid    ),
     .user_bas_mm_respndr_araddr    ( bas_axi_mm_araddr  ),
     .user_bas_mm_respndr_aruser    ( bas_axi_mm_aruser  ),
     .user_bas_mm_respndr_arlen     ( bas_axi_mm_arlen   ),
     .user_bas_mm_respndr_arsize    ( bas_axi_mm_arsize  ),
     .user_bas_mm_respndr_arburst   ( bas_axi_mm_arburst ),
     .user_bas_mm_respndr_arprot    ( bas_axi_mm_arprot  ),
     .user_bas_mm_respndr_arlock    ( bas_axi_mm_arlock  ),
     .bas_user_mm_respndr_rvalid    ( bas_axi_mm_rvalid  ),
     .bas_user_mm_respndr_rlast     ( bas_axi_mm_rlast   ),
     .user_bas_mm_respndr_rready    ( bas_axi_mm_rready  ),
     .bas_user_mm_respndr_rid       ( bas_axi_mm_rid     ),
     .bas_user_mm_respndr_rdata     ( bas_axi_mm_rdata   ),
     .bas_user_mm_respndr_rresp     ( bas_axi_mm_rresp   ),
     ...
     .axi_st_areset_n            ( axi_st_areset_n_wire   ),
     .axi_lite_areset_n          ( axi_lite_areset_n_wire ),
     .axi_mm_clk                 ( axi_mm_clk      ),
     .axi_mm_areset_n            ( axi_mm_areset_n_wire   )
...
`pragma questa_oem_00 ""
`endif