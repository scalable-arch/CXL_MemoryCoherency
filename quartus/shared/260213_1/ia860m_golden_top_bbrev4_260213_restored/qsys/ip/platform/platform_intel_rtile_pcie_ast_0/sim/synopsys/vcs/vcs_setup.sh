...
TOP_LEVEL_NAME="platform_intel_rtile_pcie_ast_0"
QSYS_SIMDIR="./../../"
...
source $QSYS_SIMDIR/common/vcs_files.tcl
set files [dict merge $files [platform_intel_rtile_pcie_ast_0::get_common_design_files "$QSYS_SIMDIR"]]
...
set QUARTUS_INSTALL_DIR [lindex $argv 2]
source $QSYS_SIMDIR/common/vcs_files.tcl
if {[catch {platform_intel_rtile_pcie_ast_0::get_design_files "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"} errorMsg]} {
 puts "Catch error: $errorMsg"
 exit 1 
}
set files [dict create]
set files [dict merge $files [platform_intel_rtile_pcie_ast_0::get_design_files "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"]]
...
run_vcs="vcs -lca -timescale=1ps/1ps -sverilog +verilog2001ext+.v $ELAB_OPTIONS $USER_DEFINED_ELAB_OPTIONS \
  ...
  $DEVICES_SIM_LIB_DIR/tennm_fp8_noc.sv \
  $DEVICES_SIM_LIB_DIR/tennm_fp8_noc_ncrypt.sv \
  ...
  $DEVICES_SIM_LIB_DIR/tennm_revb_io96.sv \
  $DEVICES_SIM_LIB_DIR/tennm_revb_io96_ncrypt.sv \
  $DEVICES_SIM_LIB_DIR/tennm_revb_io96_iopll.sv \
  $DEVICES_SIM_LIB_DIR/tennm_revb_io96_iopll_ncrypt.sv \
  $common_design_files \
  $design_files \
  $USER_DEFINED_ELAB_OPTIONS_APPEND \
  -top $TOP_LEVEL_NAME
