...
if { ! [timing_netlist_exist] } {
   ...
   source "$script_dir/platform_hbm_fp_0_hbm_arch_fp_10_n6lpfga_ip_parameters.tcl"
   source "$script_dir/platform_hbm_fp_0_hbm_arch_fp_10_n6lpfga_parameters.tcl"
   source "$script_dir/platform_hbm_fp_0_hbm_arch_fp_10_n6lpfga_pin_map.tcl"
   source "$script_dir/platform_hbm_fp_0_hbm_arch_fp_10_n6lpfga_report_timing_core.tcl"
   if { ! [timing_netlist_exist] } {
...
if [ info exists hbm_fp_db ] {
   unset hbm_fp_db
}
platform_hbm_fp_0_hbm_arch_fp_10_n6lpfga_initialize_hbm_fp_db hbm_fp_db var
...
# following loop
set instances [ array names hbm_fp_db ]
set inst_id 0
foreach inst $instances {
   ...
   if {[llength $instances] <= 1} {
      set fbasename "${::GLOBAL_platform_hbm_fp_0_hbm_arch_fp_10_n6lpfga_corename}"
   } else {
      set fbasename "${::GLOBAL_platform_hbm_fp_0_hbm_arch_fp_10_n6lpfga_corename}_${inst_id}"
   }
   ...
   platform_hbm_fp_0_hbm_arch_fp_10_n6lpfga_perform_read_capture_analysis $opcname $inst pins var summary
   
   #######################################
   # Write Analysis

   platform_hbm_fp_0_hbm_arch_fp_10_n6lpfga_perform_write_launch_analysis $opcname $inst pins var summary

   #######################################
   # Address/command Analyses

   platform_hbm_fp_0_hbm_arch_fp_10_n6lpfga_perform_ac_analysis  $opcname $inst pins var summary

   #######################################
   # DQS Gating Analysis

   platform_hbm_fp_0_hbm_arch_fp_10_n6lpfga_perform_dqs_gating_analysis   $opcname $inst pins var summary

   #######################################
   # Write Levelling Analysis 
   
   platform_hbm_fp_0_hbm_arch_fp_10_n6lpfga_perform_write_levelling_analysis $opcname $inst pins var summary

   #######################################
   # Analyses related to FPGA core
   
   platform_hbm_fp_0_hbm_arch_fp_10_n6lpfga_perform_core_analysis $opcname $inst pins var summary

   #######################################
   # Print out the Summary Panel for this instance   

   set summary [lsort -command platform_hbm_fp_0_hbm_arch_fp_10_n6lpfga_sort_proc $summary]

   set f -1
   set fname "${fbasename}_summary.csv"

   if { [platform_hbm_fp_0_hbm_arch_fp_10_n6lpfga_get_operating_conditions_number] == 0 } {
      set f [open $fname w]

      puts $f "Core: ${::GLOBAL_platform_hbm_fp_0_hbm_arch_fp_10_n6lpfga_corename} - Instance: $inst"
   ...
   post_message -type info "Core: ${::GLOBAL_platform_hbm_fp_0_hbm_arch_fp_10_n6lpfga_corename} - Instance: $inst"
   ...
   foreach summary_line $summary {
      ...
      if {$su != "--"} {
         set su [ platform_hbm_fp_0_hbm_arch_fp_10_n6lpfga_round_3dp $su]
      }
      if {$hold != "--"} {
         set hold [ platform_hbm_fp_0_hbm_arch_fp_10_n6lpfga_round_3dp $hold]
      }
   ...
   close $f
   if {$total_failures > 0} {
      post_message -type critical_warning "HBM FP Timing requirements not met"
...
set curr_speedgrade [string range $curr_part 12 14]
...
if {$curr_speedgrade != $effective_target_speedgrade} {
   post_message -type critical_warning "This HBM2E FP Interface IP core was generated for a speed grade $effective_target_speedgrade device, but the speed grade of $curr_part is $curr_speedgrade. You should regenerate the IP core to match the target device to avoid hardware issue."
}
