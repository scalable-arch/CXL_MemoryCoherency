...
module ed_synth_dut_altera_merlin_width_adapter_1933_2qdsena 
#(
    ...
    parameter PACKING                       = 1,    
    parameter CONSTANT_BURST_SIZE           = 1,    
    parameter RESPONSE_PATH                 = 0,    
...
( 
    ...
    reg out_sop_ooo_field;
    reg out_eop_ooo_field;    
   ...
   generate
         ...
         always @* begin         
            if (cmd_burst_size > OUT_NUMSYMBOLS)begin
               if(out_startofpacket && out_valid && out_ready)
                ...
                if(out_endofpacket && out_valid && out_ready)
                   out_eop_ooo_field =1;
                else
                   out_eop_ooo_field =0;
             end
      ...
      if (OUT_NUMSYMBOLS > IN_NUMSYMBOLS)  begin
         ...
         reg [IN_POISON_W-1:0]   p0_poison_field; 
         reg [IN_BYTEEN_W-1:0]   p0_byteen_field;
         reg [IN_DATACHK_W-1:0]  p0_datachk_field;
         ...
         reg [IN_POISON_W-1:0]   p0_reg_poison_field; 
         reg [IN_BYTEEN_W-1:0]   p0_reg_byteen_field;
         reg [IN_DATACHK_W-1:0]  p0_reg_datachk_field; 
         ...
         reg [IN_DATACHK_W-1:0]  p1_datachk_field; 
         ...
         wire [31:0]        int_encoded_burstsize = NW_BITFORSELECT_R; 
      ...
      reg [9:0]   size_ratio;
        ...
        if (SYNC_RESET == 0) begin : async_rst1
         ...
         always @(posedge clk) begin
               ...
               if(ROLE_BASED_USER)
               p0_reg_datachk_field  <= p0_datachk_field; 
               else
               p0_reg_datachk_field  <= 0; 
               ...
               if(ROLE_BASED_USER)
               p0_reg_poison_field           <= p0_poison_field; 
               else
               p0_reg_poison_field           <= '0; 
         end
         ...
         always @* begin
            ...
            if (ROLE_BASED_USER) 
               p0_poison_field   = in_poison_field; 
            else 
               p0_poison_field   = '0;
            
            p0_bwrap_field    = in_burstwrap_field;
            p0_byteen_field   = in_byteen_field;
            p0_address_field  = address_for_adaptation;  
            if (ROLE_BASED_USER) 
              p0_datachk_field  = in_datachk_field; 
            else 
              p0_datachk_field  = '0;
            ...
            if (p0_use_reg) begin
               ...
               if (ROLE_BASED_USER) begin
                  p0_datachk_field  = p0_reg_datachk_field;
         ...
         if ((RESPONSE_PATH == 0) && (PACKING == 1)) begin 
         ...
         always @* begin
            ...
            if (ROLE_BASED_USER)
               p1_poison_field   = p0_poison_field; 
            else 
               p1_poison_field   = '0; 
            ...
            if (ROLE_BASED_USER) 
               p1_datachk_field  = p0_datachk_field; 
            else 
               p1_datachk_field  = '0;
         ...
         always @* begin
            ...
            if (RESPONSE_PATH == 0) begin
                if (PACKING == 1) begin 
                    ...
                    out_size_field      = out_numsymbols_wire[BURST_SIZE_W-1:0]; 
                 end else begin
   ...
   always @* begin
      ...
      if( ENABLE_OOO ) begin
         out_data[OUT_PKT_SOP_OOO]                                       = out_sop_ooo_field;
         out_data[OUT_PKT_EOP_OOO]                                       = out_eop_ooo_field; 
      end      
      if (ROLE_BASED_USER) begin
...
`pragma questa_oem_00 ""
`endif