...
필요하면 각 poll 사이에 sleep 하며, 실패 시 timeout을 반환한다.
*/
...
해당 값을 0으로 지워 다음 실행에서도 같은 핸드셰이크 영역을 재사용할 수 있게 한다.
*/
...
// 중요: 먼저 메모리를 해제하고, 그 다음 디바이스를 닫는다.
/*
자원 정리 전용 함수.
VFIO로 할당한 메모리 객체들을 안전한 순서로 먼저 해제하고,
마지막에 디바이스 핸들을 닫는다.
*/
...
/*
사용법 출력 함수.
호스트 증가 횟수와 PCIe BDF 선택을 위한
실행 옵션 형식을 사용자에게 보여준다.
*/
...
/*
메인 함수.
명령행 인자를 파싱하고, VFIO와 공유 DMA 버퍼를 초기화한 뒤,
호스트-디바이스 핸드셰이크, 카운터 증가 작업, 완료 대기, 결과 출력,
