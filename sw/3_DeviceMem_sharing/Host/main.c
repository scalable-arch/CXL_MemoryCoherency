/*
Explanation
이 코드는 호스트가 DMA를 통해 디바이스 메모리의 카운터와 핸드셰이크 영역에 접근하는 예제이다.
...
자신의 lock 의사를 먼저 표시하고,
상대가 이미 진입 의사를 가진 경우 turn 값을 보며 양보와 재시도를 수행한다.
*/
...
turn을 디바이스 쪽으로 넘기고,
호스트 lock flag를 내려 임계 구역 사용 종료를 알린다.
*/
...
현재 핸드셰이크 블록을 읽은 뒤
REFRESH_REQ를 token으로 세팅하고 REFRESH_ACK는 0으로 초기화한다.
*/
...
/*
프로그램 사용법을 출력하는 함수.
명령행 옵션으로 설정 가능한 increment 횟수와
PCI BDF 인자 형식을 사용자에게 보여준다.
*/
